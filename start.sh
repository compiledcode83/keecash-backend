...
local_start() {
    ...
    tmux send-keys -t KEECASH:api.0 'npm run api:start:prod --port=3000' C-m
    # REST API: Webhook API Handler
    tmux send-keys -t KEECASH:api.1 'npm run webhook:start:prod --port=3001' C-m
    # REST API: Admin Panel API Service
    tmux send-keys -t KEECASH:api.2 'npm run admin:start:prod --port=3002' C-m

    # Kafka: Kafka Event Producer Service
    tmux send-keys -t KEECASH:kafka.0 'npm run producer:start:prod --port=3003'  C-m
    # Kafka: Kafka Event Consumer Service
    tmux send-keys -t KEECASH:kafka.1 'npm run consumer:start:prod' C-m
